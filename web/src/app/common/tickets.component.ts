...
const Airlines = {
    ...
    "MU": ["中国东方航空公司", "95530", "http://www.ceair.com/"],
    ...
    "UA": ["美国联合航空公司", "4008-810-213", "https://www.united.com"],
    "LV": ["美佳环球航空", "", ""],
    ...
    <mat-expansion-panel *ngFor="let airline of airlines" (afterExpand)="open(airline)" #panel>
        <mat-expansion-panel-header [collapsedHeight]="'auto'" [expandedHeight]="'auto'">
            <mat-list-item  >
                ...
                <h6 mat-line >
                    航班:
                    <span *ngFor="let flight of flights(airline.flight); let i = index">
                        <button mat-icon-button class="small" (click)="link(flight)" color="warn">
                            <img style="width:15px" src={{company(flight)}}>
                        </button>{{cname(flight)}}|
                        <span *ngFor="let f of airline.flight.split(',')" >
                            {{f}}
                        </span>
                    </span>
                </h6>
...
export class TicketComponent implements OnInit {
    ...
    getSubTicketList(airline, pg: number, opened: boolean = false){
        if(!opened) {
            this.hr.post('ticket/sublist', { page: pg, end: airline.E, begin:airline.B, note: this.arrives, contains:this.contains, eqe: this.filter1.input, stops:this.filter2.input }, result => {
    ...
    getSubTicketCount(airline, opened: boolean = false){
        if(!opened) {
            this.hr.post('ticket/subcount', { end: airline.E, begin:airline.B, note: this.arrives, contains:this.contains, eqe: this.filter1.input, stops:this.filter2.input }, result => {
